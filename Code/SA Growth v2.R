...
################################################################
##### Rerun April 23, 2024 with updated metabolic rates#########
################################################################
...
for (rep_num in c(1:sample_size)) {
  ...
  masses_rep <- masses %>%
    ...
    # randomly vary the Growth variable by sd
        # mutate(Growth = rnorm(1, Growth, sd = 0.05)) %>%
...
data.true <- model_all_reps %>% 
  filter(rep_num == 1) %>% 
  select(Age, Sex, with.pup, total_energy, all_of(change_var))

#All replicates except original in one table
data.reps <- model_all_reps %>% 
  filter(rep_num != 1) %>% 
  select(Age, Sex, with.pup, total_energy, all_of(change_var), rep_num)
