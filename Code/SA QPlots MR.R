...
mods <- read.csv("model_all_reps_MR_0.1.csv")
...
(MR_Qplot_0.1<-
   ...
   theme_bw()+
   ggtitle("Metabolic Rate, stdev 10%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("MR_Qplot_10.jpeg", width = 6, height = 3)

#---- Qplot 5% ----
mods <- read.csv("model_all_reps_MR_0.05.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(MR_Qplot_0.05<-
   ...
   geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
   theme_bw()+
   ggtitle("Metabolic Rate, stdev 5%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("MR_Qplot_05.jpeg", width = 6, height = 3)

#---- Qplot 20% ----
mods <- read.csv("model_all_reps_MR_0.2.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(MR_Qplot_0.2<-
   ...
   geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
   theme_bw()+
   ggtitle("Metabolic Rate, stdev 20%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("MR_Qplot_20.jpeg", width = 6, height = 3)


#---- Qplot stdev from Lit----
mods <- read.csv("model_all_reps_MR_lit.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(
  MR_Qplot_lit <-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Metabolic Rate, stdev from literature")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("MR_Qplot_lit.jpeg", width = 6, height = 3)
