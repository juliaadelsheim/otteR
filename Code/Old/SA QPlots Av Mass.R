...
mods <- read.csv("model_all_reps_Av_mass_0.1.csv")
...
(Av_mass_Qplot_0.1<-
   ...
   theme_bw()+
   ggtitle("Average Mass, stdev 10%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("Av_mass_Qplot_10.jpeg", width = 6, height = 3)

#---- Qplot 5% ----
mods <- read.csv("model_all_reps_Av_mass_0.05.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(Av_mass_Qplot_0.05<-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Average Mass, stdev 5%")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("Av_mass_Qplot_05.jpeg", width = 6, height = 3)
...
(Av_mass_Qplot_0.20<-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Average Mass, stdev 20%")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("Av_mass_Qplot_20.jpeg", width = 6, height = 3)
...
(
  Growth_Qplot_lit <-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Average Mass, stdev from literature")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("Av_mass_Qplot_lit.jpeg", width = 6, height = 3)
