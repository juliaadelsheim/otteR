...
mods <- read.csv("model_all_reps_perc_time_0.1.csv")
...
(perc_time_Qplot_0.1<-
   ...
   theme_bw()+
   ggtitle("Percent Time, stdev 10%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("perc_time_Qplot_10.jpeg", width = 6, height = 3)

#---- Qplot 5% ----
mods <- read.csv("model_all_reps_perc_time_0.05.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
#---- Qplot 5% by AMBRE----
(perc_time_Qplot_0.05<-
   ggplot(data=range, aes(colour=as.factor(Sex), fill=as.factor(Sex), linetype=as.factor(with.pup))) + 
   geom_line(aes(Age, mean)) +
   geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
   theme_bw()+
   ggtitle("Percent Time, stdev 5%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("perc_time_Qplot_05.jpeg", width = 6, height = 3)

#---- Qplot 20% ----
mods <- read.csv("model_all_reps_perc_time_0.2.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(perc_time_Qplot_0.2<-
   ...
   geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
   theme_bw()+
   ggtitle("Percent Time, stdev 20%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("perc_time_Qplot_20.jpeg", width = 6, height = 3)


#---- Qplot stdev from Lit----
mods <- read.csv("model_all_reps_perc_time_lit.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(
  perc_time_Qplot_lit <-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Percent Time, stdev from literature")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("perc_time_Qplot_lit.jpeg", width = 6, height = 3)
