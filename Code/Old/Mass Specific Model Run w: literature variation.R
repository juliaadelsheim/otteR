...
male_TEE_MS <- test %>% 
  select(Sex, Age, total_energy, rep_num) %>% 
  filter(Sex == "M") %>%  
  mutate(total_energy = total_energy/(mass_m$Av_mass))

male_TEE_MS_ave <- male_TEE %>% 
  group_by(Age) %>%
  summarize(mean_energy = mean(total_energy),
            stdev = sd(total_energy))
...
female_nopup_TEE_MS <- test %>%
  select(Age, Sex, with.pup, total_energy,rep_num) %>%
  filter(Sex == "F", with.pup == "no") %>% 
  mutate(total_energy = total_energy/(massf$Av_mass))

female_nopup_TEE_MS_ave <- female_nopup_TEE %>% 
  group_by(Age) %>%
  summarize(mean_energy = mean(total_energy),
            stdev = sd(total_energy)) 
...
female_withpup_TEE_MS <- test %>%
  select(Age, Sex, with.pup, total_energy, rep_num) %>%
  filter(Sex == "F", with.pup == "yes") %>% 
  mutate(total_energy = total_energy/(massfwp$Av_mass))

female_withpup_TEE_MS_ave <- female_withpup_TEE %>% 
  group_by(Age) %>%
  summarize(mean_energy = mean(total_energy),
            stdev = sd(total_energy)) 
...
write.xlsx(female_nopup_TEE_MS_ave, file = "MSmodel_with_varFNP.xlsx",
           sheetName = "Female No Pup ", append = TRUE)
# Add a second data set in a new worksheet
write.xlsx(female_withpup_TEE_MS_ave, file = "MSmodel_with_varFWP.xlsx", 
           sheetName="Female With Pup", append=TRUE)
# Add a third data set
write.xlsx(male_TEE_MS_ave, file = "MSmodel_with_varM.xlsx",
