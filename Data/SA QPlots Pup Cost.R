...
mods <- read.csv("model_all_reps_3931_0.1.csv")
...
(pup_cost_Qplot_0.1<-
   ...
   theme_bw()+
   ggtitle("Pup Cost, stdev 10%")+
   ylab("Energy Requirements (kJ/day)")
)
ggsave ("pup_cost_Qplot_10.jpeg", width = 6, height = 3)

#---- Qplot 5% ----
mods <- read.csv("model_all_reps_3931_0.05.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(pup_cost_Qplot_0.05<-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Pup Cost, stdev 5%")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("pup_cost_Qplot_05.jpeg", width = 6, height = 3)

#---- Qplot 20% ----
mods <- read.csv("model_all_reps_3931_0.2.csv")
# unique(model_all_reps$Sex)
# fem <- model_all_reps[model_all_reps$Sex=='F' & model_all_reps$with.pup=='yes',]
ggplot(mods, aes(Age, total_energy, shape=Sex, colour=rep_num)) + geom_point()
...
(pup_cost_Qplot_0.20<-
    ...
    geom_ribbon(aes(x=Age, ymin=Q5, ymax=Q95), colour=NA, alpha=.3) +
    theme_bw()+
    ggtitle("Pup Cost, stdev 20%")+
    ylab("Energy Requirements (kJ/day)")
)
ggsave ("pup_cost_Qplot_20.jpeg", width = 6, height = 3)
